...
const I = {
  ...
  todo: <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>,
...
const SIDEBAR_LIFE = [
  { to: "/todo", label: "To Do", icon: I.todo },
  { to: "/calendar", label: "Calendar", icon: I.cal },
  { to: "/email", label: "Email", icon: I.email },
];
const SIDEBAR_SYS = [
  { to: "/notes", label: "Notes", icon: I.notes },
  { to: "/automations", label: "Auto", icon: I.auto },
  { to: "/tools", label: "Tools", icon: I.tools },
  { to: "/settings", label: "Settings", icon: I.set },
];

// Mobile bottom nav — just the key 5
const MOBILE_NAV = [
  { to: "/", label: "Home", icon: I.home, end: true },
  { to: "/todo", label: "To Do", icon: I.todo },
  { to: "/calendar", label: "Calendar", icon: I.cal },
  { to: "/email", label: "Email", icon: I.email },
  { to: "/settings", label: "More", icon: I.set },
];
...
function Shell({ onLogout, profile }: { onLogout: () => void; profile: any }) {
  ...
  const sideLink = (n: { to: string; label: string; icon: React.ReactNode; end?: boolean }) => (
    ...
    <div className="flex flex-col md:flex-row h-screen" style={{ background: "var(--bg-root)" }}>
      ...
      <aside className="hidden md:flex w-[60px] flex-col items-center py-3 border-r shrink-0" style={{ background: "var(--bg-surface)", borderColor: "var(--border)" }}>
        ...
        <nav className="flex-1 flex flex-col items-center w-full">
          {SIDEBAR_MAIN.map(sideLink)}
          <div className="w-6 my-1.5" style={{ borderTop: "1px solid var(--border)" }} />
          {SIDEBAR_LIFE.map(sideLink)}
          <div className="w-6 my-1.5" style={{ borderTop: "1px solid var(--border)" }} />
          {SIDEBAR_SYS.map(sideLink)}
        </nav>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="shrink-0 border-b px-4 py-2 flex items-center gap-3" style={{ background: "var(--bg-root)", borderColor: "var(--border)" }}>
          ...
          <div className="flex-1 max-w-xs mx-auto relative">
            ...
            {showSearch && searchR.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 rounded-lg border shadow-lg max-h-56 overflow-auto z-50" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
                {searchR.map((r, i) => (
                  <div key={i} className="px-3 py-2 text-xs border-b hover:brightness-110" style={{ borderColor: "var(--border)" }} onClick={() => { setShowSearch(false); setSearchQ(""); }}>
          ...
          <div className="relative">
            <button onClick={async () => { setNotifs(await api.getNotifications()); setShowNotifs(!showNotifs); }} className="relative p-1" style={{ color: "var(--text-muted)" }}>
              ...
              {notifCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] flex items-center justify-center font-bold" style={{ background: "var(--accent)", color: "#000" }}>{notifCount > 9 ? "9+" : notifCount}</span>}
            </button>
            {showNotifs && (
              <div className="absolute right-0 top-full mt-1 w-72 rounded-lg border shadow-lg max-h-72 overflow-auto z-50" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
                ...
                {notifs.map((n) => (
                  <div key={n.id} className="px-3 py-2 border-b" style={{ borderColor: "var(--border)", background: n.read ? "transparent" : "var(--accent-bg)" }} onClick={async () => { await api.readNotif(n.id); loadNotifCount(); setNotifs(notifs.map((x) => x.id === n.id ? { ...x, read: true } : x)); }}>
        ...
        <div className={`flex-1 overflow-auto ${isHome ? "" : "p-4 md:p-6"}`} style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
          <Routes>
            ...
            <Route path="/tools" element={<Modules />} />
        ...
        <nav className="md:hidden shrink-0 border-t flex justify-around py-1" style={{ background: "var(--bg-surface)", borderColor: "var(--border)", paddingBottom: "env(safe-area-inset-bottom, 8px)" }}>
          {MOBILE_NAV.map((n) => (
            <NavLink key={n.to} to={n.to} end={n.end} className="flex flex-col items-center py-1 px-2 min-w-[48px]">
              {({ isActive }) => (
                <>
                  <span style={{ color: isActive ? "var(--accent)" : "var(--text-muted)" }}>{n.icon}</span>
                  <span className="text-[9px] mt-0.5" style={{ color: isActive ? "var(--accent)" : "var(--text-muted)" }}>{n.label}</span>
                </>
              )}
            </NavLink>
          ))}
...
function SupportModal({ onClose }: { onClose: () => void }) {
  ...
  const [subject, setSubject] = useState(""); const [description, setDescription] = useState(""); const [category, setCategory] = useState("General");
  const [sending, setSending] = useState(false); const [result, setResult] = useState<any>(null);
  const submit = async () => { if (!subject.trim() || !description.trim()) return; setSending(true); try { const r = await fetch("/api/support/ticket", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subject, description, category }) }); setResult(await r.json()); } finally { setSending(false); } };
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="w-full md:max-w-lg rounded-t-2xl md:rounded-xl border p-5 max-h-[85vh] overflow-auto" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="text-sm p-1" style={{ color: "var(--text-muted)" }}>Close</button>
        </div>
          ...
          <div className="space-y-3 text-sm">
            <div className="rounded-lg p-3 border" style={{ borderColor: "var(--border)", background: "var(--bg-input)" }}>
              ...
              <ul className="list-disc list-inside space-y-1 text-xs" style={{ color: "var(--text-muted)" }}>
                <li><strong>Home</strong> — Chat with your assistant, attach files</li>
                <li><strong>To Do</strong> — Tasks by category (Work, Personal, School...)</li>
                <li><strong>Calendar</strong> — Events and schedule</li>
                <li><strong>Email</strong> — Read, send, reply</li>
                <li><strong>Notes</strong> — Pin important info</li>
                <li><strong>Automations</strong> — Recurring workflows</li>
                <li><strong>Tools</strong> — Install add-ons</li>
              </ul>
            </div>
            <div className="rounded-lg p-3 border" style={{ borderColor: "var(--border)", background: "var(--bg-input)" }}>
              <p className="font-medium mb-2 text-xs" style={{ color: "var(--text-primary)" }}>Try saying...</p>
              <ul className="space-y-1 text-xs" style={{ color: "var(--text-muted)" }}>
                <li>"Add a task: buy groceries by Friday, high priority"</li>
                <li>"Remind me to call the doctor tomorrow at 3pm"</li>
                <li>"Check my email and summarize it"</li>
                <li>"Save a note: wifi password is abc123"</li>
              </ul>
            </div>
            <div className="rounded-lg p-3 border" style={{ borderColor: "var(--border)", background: "var(--bg-input)" }}>
              <p className="font-medium mb-2 text-xs" style={{ color: "var(--text-primary)" }}>Setup guides</p>
              <ul className="space-y-1 text-xs" style={{ color: "var(--text-muted)" }}>
                <li><strong>Telegram</strong> — Message @BotFather, create bot, paste token in Settings, pair via agent Telegram tab</li>
                <li><strong>Email</strong> — Add IMAP/SMTP details in Settings. Gmail: use App Password</li>
                <li><strong>Tools</strong> — Browse and install add-ons with one click</li>
              </ul>
            </div>
          </div>
        )}
        {tab === "ticket" && (
          <div className="space-y-3">
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" style={{ background: "var(--bg-input)", borderColor: "var(--border)", color: "var(--text-primary)" }}><option>General</option><option>Bug report</option><option>Feature request</option><option>Configuration help</option></select>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe..." className="w-full rounded-md border px-3 py-2 text-sm min-h-[80px]" style={{ background: "var(--bg-input)", borderColor: "var(--border)", color: "var(--text-primary)" }} />
            ...
            <button onClick={submit} disabled={sending || !subject.trim() || !description.trim()} className="w-full py-2.5 rounded-md text-sm font-medium disabled:opacity-30" style={{ background: "var(--accent)", color: "#000" }}>{sending ? "..." : "Submit"}</button>
