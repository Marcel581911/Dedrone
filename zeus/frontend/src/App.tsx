...
export default function App() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!s.setup) { setAuthState("setup"); return; }
      if (!s.authenticated) { setAuthState("login"); return; }
      setProfile(s.user);
  ...
  if (authState === "loading") return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "var(--bg-root)" }}>
      <div className="text-2xl font-semibold" style={{ color: "var(--accent)" }}>GULLI</div>
    </div>
  );
  if (authState === "setup") return <Setup onComplete={() => checkAuth()} />;
...
function Shell({ onLogout, profile }: { onLogout: () => void; profile: any }) {
  const assistantName = profile?.assistantName || "Gulli";
  ...
  const loadNotifCount = useCallback(() => {
    api.notifCount().then((r) => setNotifCount(r.count)).catch(() => {});
  }, []);

  useEffect(() => {
    loadNotifCount();
    const t = setInterval(loadNotifCount, 10000);
    return () => clearInterval(t);
  }, [loadNotifCount]);

  const doSearch = async () => {
    if (searchQ.length < 2) return;
    const r = await api.search(searchQ);
    setSearchR(r.results || []);
    setShowSearch(true);
  };
    ...
    <div className="flex flex-col md:flex-row h-screen" style={{ background: "var(--bg-root)" }}>
      ...
      <aside className="hidden md:flex w-[60px] flex-col items-center py-3 border-r shrink-0" style={{ background: "var(--bg-surface)", borderColor: "var(--border)" }}>
        ...
        <div className="mt-auto flex flex-col items-center gap-1 mb-2">
          <button onClick={() => setShowSupport(true)} title="Help" className="opacity-30 hover:opacity-70">
          ...
          <button onClick={async () => { await api.logout(); onLogout(); }} title="Sign out" className="opacity-30 hover:opacity-70">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="shrink-0 border-b px-4 py-2 flex items-center gap-3" style={{ background: "var(--bg-root)", borderColor: "var(--border)" }}>
          ...
          <div className="flex-1 max-w-xs mx-auto relative">
            <input value={searchQ} onChange={(e) => setSearchQ(e.target.value)}
              ...
              onFocus={() => searchR.length > 0 && setShowSearch(true)}
              placeholder="Search..." className="w-full rounded-md border px-3 py-1.5 text-xs"
            ...
            {showSearch && searchR.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 rounded-lg border shadow-lg max-h-56 overflow-auto z-50" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
                {searchR.map((r, i) => (
                  <div key={i} className="px-3 py-2 text-xs border-b hover:brightness-110 cursor-pointer" style={{ borderColor: "var(--border)" }} onClick={() => { setShowSearch(false); setSearchQ(""); }}>
          ...
          <div className="relative">
            ...
            {showNotifs && (
              <div className="absolute right-0 top-full mt-1 w-72 rounded-lg border shadow-lg max-h-72 overflow-auto z-50" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
                ...
                {notifs.map((n) => (
                  <div key={n.id} className="px-3 py-2 border-b cursor-pointer" style={{ borderColor: "var(--border)", background: n.read ? "transparent" : "var(--accent-bg)" }}
                    onClick={async () => { await api.readNotif(n.id); loadNotifCount(); setNotifs(notifs.map((x) => x.id === n.id ? { ...x, read: true } : x)); }}>
        ...
        <div className={`flex-1 overflow-auto ${location.pathname === "/" ? "" : "p-4 md:p-6"}`} style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
          <Routes>
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="w-full md:max-w-lg rounded-t-2xl md:rounded-xl border p-5 max-h-[85vh] overflow-auto" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-1 mb-4 border-b" style={{ borderColor: "var(--border)" }}>
          {[["help", "Help"], ["ticket", "Submit Ticket"]].map(([k, l]) => (
            <button key={k} onClick={() => { setTab(k as any); setResult(null); }} className="px-3 py-2 text-xs font-medium relative" style={{ color: tab === k ? "var(--accent)" : "var(--text-muted)" }}>
              {l}{tab === k && <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "var(--accent)" }} />}
            </button>
          ))}
        </div>
        {tab === "help" && (
        ...
        {tab === "ticket" && (
          <div className="space-y-3">
            <select value={cat} onChange={(e) => setCat(e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" style={{ background: "var(--bg-input)", borderColor: "var(--border)", color: "var(--text-primary)" }}>
              <option>General</option><option>Bug report</option><option>Feature request</option>
            </select>
            ...
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Describe..." className="w-full rounded-md border px-3 py-2 text-sm min-h-[80px]" style={{ background: "var(--bg-input)", borderColor: "var(--border)", color: "var(--text-primary)" }} />
            ...
            <button onClick={submit} disabled={sending || !subject.trim() || !desc.trim()} className="w-full py-2.5 rounded-md text-sm font-medium disabled:opacity-30" style={{ background: "var(--accent)", color: "#000" }}>{sending ? "..." : "Submit"}</button>
          </div>
        )}
      </div>
    </div>
